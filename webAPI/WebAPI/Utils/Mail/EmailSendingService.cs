...
namespace WebAPI.Utils.Mail
{
    public class EmailSendingService
    {
        ...
        public async Task SendWelcomeEmail(string email, string userName)
        {
            try
            {
                MailRequest request = new MailRequest { ToEmail = email, Body = GetHtmlContent(userName), Subject = "Bem vindo ao vital Hub" };
        ...
        public async Task SendRecoveryEmail(string email, int codigo)
        {
            try
            {
                MailRequest request = new MailRequest { ToEmail = email, Body = GetHtmlContentRecovery(codigo), Subject = "VitalHub - Esqueceu a Senha" };
                await emailService.SendEmailAync(request);
